...
tri.agg <- aggregate(tri, fact = 20, fun=max) #change this

#create a concave up functional form - the size of the exponent controls when the 'take-off' happens and how steeply
tri.exp.up.resist <- (1 + tri.agg)^5
plot(tri.agg, tri.exp.up.resist)

#create a concave down functional form, the smaller the decimal, the quicker you reach the asymptote
tri.exp.dn.resist <- (1 + tri.agg)^.01
plot(tri.agg, tri.exp.dn.resist)
```

Note that `gdistance` uses transition layers that are built on conductance (the inverse of resistance) so we need to invert our resistance surface once we've built it. It also requires a special object called a `TransitionLayer`. We accomplish both of those in the call to `transition` below.
